...
| **Backends** | `src/utils/swarm/backends/` | 可插拔执行后端：`tmux` / `iterm2` / `in-process`（registry 分发）——**OpsHive 新增 `opshive` 后端**：teammates 真实执行 + 消息镜像到控制平面 |
...
3. **镜像**：`opshive` backend 把消息镜像到控制平面证据总线 → 会议室可视化 + IOM 结构化。
4. **多机**：`bridge:` 跨会话/跨机（ListPeers 发现），跨机消息需用户同意。

## 许可注意

该仓库为 Claude Code 的 fork，接入 OpsHive 时须核对其开源许可条款（MIT/企业许可边界），开源分发时在 NOTICE 中声明。
